...
def main(plot_name):
    input_dir = f"May_results/{plot_name}"
    ...
    for measurement in measurements:
        ...
        for wheel, (x, y) in wheels.items():
            ...
            wheel_coords[wheel] = calculate_new_gps(lat, lon, range_m, np.rad2deg(bearing_rad % (2 * np.pi)))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Project May robot components onto GPS map.")
