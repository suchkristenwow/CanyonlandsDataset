...
def main(plot_name, config_path):
    config = toml.load(config_path)

    workspace_path = config["paths"]["workspace_path"]
    season_dir = config["paths"]["season_dir"]
    ...
    gps_fix_cmd = source_cmd + f"python3 publish_gps_fix.py"
    imu_cmd = source_cmd + f"python3 publish_IMU_correct.py"
    frame_cmd = source_cmd + f"python3 get_frame_gps_coords.py --season May --plot_name {plot_name}"
    ...
    if not os.path.exists(os.path.join(results_dir,"yawHeading_data.csv")) or not file_has_data(os.path.join(results_dir,"yawHeading_data.csv")): 
        ...
        liosam_cmd = source_cmd + f"roslaunch {workspace_path}/src/LIO-SAM/launch/run_restorebot_May.launch"
        ...
        print(f"▶️ Playing bag for {season}/{plot_name}")
        bag_proc = run_bag(bag_dir, plot_name, "May", topics=[
            "/nmea_sentence",
            "/H03/imu/data",
    ...
    # Step 8: Run getGPSCoords.py
    gps_coords_cmd = f"python3 {os.path.join(script_dir, 'getGPSCoords.py')} --plot_name {plot_name}"
    ...
    uncertainty_cmd = source_cmd + f"python3 gps_uncertainty_estimation.py --season May --plot_name {plot_name}"
    uncertainty_proc = launch_process(uncertainty_cmd, env=ros_env)

    bag_proc2 = run_bag(bag_dir, plot_name, "May", topics=["/nmea_sentence"]) 
    bag_proc2.wait()
